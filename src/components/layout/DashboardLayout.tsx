...
import { LayoutDashboard, Package, Users, BarChart3, Settings, LogOut, Sparkles, ChevronDown, Building2 } from "lucide-react";
const menuItems = [{
  title: "Dashboard",
  url: "/dashboard",
...
const DashboardLayout = ({
  ...
  if (!tenant) {
    return <div className="min-h-screen flex items-center justify-center bg-background">
        <OnboardingDialog open={true} onOpenChange={() => {
      ...
      <div className="min-h-screen flex w-full bg-background">
        <Sidebar className="border-r border-border/50">
          <SidebarHeader className="p-4 border-b border-border/50">
            ...
            {tenant && tenants.length > 0 && <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-56">
                  ...
                  {tenants.map(t => <DropdownMenuItem key={t.id} onClick={() => switchTenant(t.id)} className={t.id === tenant.id ? "bg-accent" : ""}>
                      {t.name}
                    </DropdownMenuItem>)}
                </DropdownMenuContent>
              </DropdownMenu>}
                ...
                <SidebarMenu>
                  {menuItems.map(item => <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton asChild>
                        <NavLink to={item.url} end={item.url === "/dashboard"} className="flex items-center gap-3 px-3 py-2 rounded-md transition-colors hover:bg-accent" activeClassName="bg-accent text-accent-foreground font-medium">
